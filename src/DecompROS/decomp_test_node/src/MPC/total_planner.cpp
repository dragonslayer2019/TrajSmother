...
void poseArrayCallback(const geometry_msgs::PoseArray::ConstPtr& msg) {
  ROS_INFO("Received PoseArray message with %ld poses", msg->poses.size());
  if (glob_path.empty()) {
    for (const auto& pose : msg->poses) {
      Eigen::Matrix<double, 3, 1> temp_pose;
      temp_pose.x() = pose.position.x;
      temp_pose.y() = pose.position.y;
      temp_pose.z() = pose.position.z;
      glob_path.push_back(temp_pose);
    }
  }
}

void obsArrayCallback(const geometry_msgs::PoseArray::ConstPtr& msg) {
  ROS_INFO("Received obsArray message with %ld obs", msg->poses.size());
  if (obs.empty()) {
    for (const auto& pose : msg->poses) {
      Eigen::Matrix<double, 3, 1> temp_pose;
      temp_pose.x() = pose.position.x;
...
int main(int argc, char ** argv){
  ...
  ros::Subscriber sub = nh.subscribe("/global_path", 10, poseArrayCallback);
  ros::Subscriber obs_sub = nh.subscribe("/obs_vec", 10, obsArrayCallback);

  ros::Publisher cloud_pub = nh.advertise<sensor_msgs::PointCloud>("cloud", 10, true);
  ros::Publisher path_pub = nh.advertise<nav_msgs::Path>("path", 10, true);
  ros::Publisher ref_path_pub = nh.advertise<nav_msgs::Path>("ref_path_path", 10, true);
  ros::Publisher smoothed_path_pub = nh.advertise<nav_msgs::Path>("smooth_path", 10, true);
  ros::Publisher es_pub = nh.advertise<decomp_ros_msgs::EllipsoidArray>("ellipsoid_array", 10, true);
  ros::Publisher poly_pub = nh.advertise<decomp_ros_msgs::PolyhedronArray>("polyhedron_array", 10, true);
  ros::Publisher marker_pub = nh.advertise<visualization_msgs::Marker>("visualization_marker", 10);
  ros::Publisher marker_pub1 = nh.advertise<visualization_msgs::Marker>("visualization_marker1", 10);
  ros::Publisher marker_pub2 = nh.advertise<visualization_msgs::Marker>("visualization_marker2", 10);
  ros::Publisher marker_pub3 = nh.advertise<visualization_msgs::Marker>("visualization_marker3", 10);
  ...
  v_points.scale.x = v_points1.scale.x = v_points2.scale.x = v_points3.scale.x = 0.3;
  v_points.scale.y = v_points1.scale.y = v_points2.scale.y = v_points3.scale.y = 0.3;
  ...
  while (ros::ok()) {
    ...
    if (!glob_path.empty()) {
      ...
      // 数据清洗
      // ROS_INFO("clean global path");


      for (int i = 0; i < glob_path.size()-1; i++) {
        for (int j = i+1; j < glob_path.size()-1; j++) {
          if (glob_path[i] == glob_path[j] || (abs(glob_path[i].x() - glob_path[j].x()) < 0.2 && abs(glob_path[i].y() - glob_path[j].y()) < 0.2 && abs(glob_path[i].z() - glob_path[j].z()) < 0.2)) {
            glob_path.erase(glob_path.begin() + j);
          } else {
            break;
          }
        } 
      ...
      // 将全局路径点均匀采样
      // ROS_INFO("sample global path");
      // double global_step = 3.0;
      // vec_Vec3f sampled_glob_path = interpolateGlobalPoints(glob_path, global_step);
      // for (const auto& point : sampled_glob_path) {
      //   std::cout << point.x() << " " << point.y() << " " << point.z() << std::endl;
      // }
      

      // 当前位置对全局路径投影找到最近点为截取起点，将轨迹下一时刻位置设为当前位置
      Eigen::Matrix<double, 3, 1> cur_pose = glob_path.front();
      // int cut_index = 1;
      // Eigen::Matrix<double, 3, 1> path_begin_point;
      if (flag) {
        // path_begin_point = sampled_glob_path.front();
      } else {
        // cur_pose.x() = res_traj.v[5](0, 0);
        // cur_pose.y() = res_traj.v[5](1, 0);
        // cur_pose.z() = res_traj.v[5](2, 0);
        cur_pose.x() = res.v[5](0, 0);
        cur_pose.y() = res.v[5](1, 0);
        cur_pose.z() = res.v[5](2, 0);
        // path_begin_point = findClosestProjectionPoint(sampled_glob_path, cur_pose, cut_index);
      }

      vec_Vec3f path = glob_path;


      // path.push_back(path_begin_point);
      // float len = distance(path_begin_point.cast<float>(), sampled_glob_path[cut_index].cast<float>());
      // // for (int i = cut_index; i < cut_index + 2 && i < sampled_glob_path.size(); i++) {
      // //   path.push_back(sampled_glob_path[i]);
      // // }
      // for (int i = cut_index; i < sampled_glob_path.size()-1; i++) {
      //   path.push_back(sampled_glob_path[i]);
      //   if (len > 1.5*HorizonNum * max_length) {
      //     break;
      //   }
      //   len += distance(sampled_glob_path[i].cast<float>(), sampled_glob_path[i+1].cast<float>());
      // }
      ...
      Eigen::Matrix<double, 3, 1> path_begin_point;
      int cut_index = 1;
      path_begin_point = findClosestProjectionPoint(glob_path, cur_pose, cut_index);
      std::vector<Eigen::Vector3f> cur_path_f(path_f.begin() + cut_index, path_f.begin() + path_f.size());
      ...
      // }
      for (int i = 0; i <= HorizonNum; ++i) {
        int idx = findClosestSegment(ref_points[i], path_f);
        std::cout << "point " << i << " is on " << idx << " lane" << std::endl;
        mpc_polyhedrons.push_back(polyhedrons[idx]);
        elliE[i] = E_inverse[idx];
        new_centerX[i].block(0, 0, 3, 1) = E_inverse_d[idx];
        Eigen::Vector3f vk = (ref_path[i+1] - ref_path[i]).cast<float>().normalized()*v_norm[i];
        Eigen::Matrix<float, 3, 1> v_center;
        v_center << vk[0], vk[1], vk[2]; 
        new_centerX[i].block(3, 0, 3, 1) = v_center;
      ...
      if (flag) {
        ...
        init_vel.x() = traj_ref_speed[5].x();
        init_vel.y() = traj_ref_speed[5].y();
        init_vel.z() = traj_ref_speed[5].z();
      }
      ...
      v_points.points.clear();
      v_points1.points.clear();
      v_points2.points.clear();
      v_points3.points.clear();
      line_strip.points.clear();
      line_strip1.points.clear();
      ...
      ros::Publisher traj_pub = nh.advertise<geometry_msgs::PointStamped>("point_to_visualize", 100);
      int count = 0;
      ...
      // while (1) {
        ...
        trj_point.point.x = res.v[count](0, 0);
        trj_point.point.y = res.v[count](1, 0);
        trj_point.point.z = res.v[count](2, 0);
        ...
        odom_trans.transform.translation.x = res_points.front().x();
        odom_trans.transform.translation.y = res_points.front().y();
        odom_trans.transform.translation.z = res_points.front().z();
